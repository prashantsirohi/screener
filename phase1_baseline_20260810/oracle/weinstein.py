...
def load_bars(path: Path) -> pd.DataFrame | None:
    """Load one cached chart JSON into a clean weekly OHLCV frame (adjusted)."""
    try:
        d = json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception:
        return None
    if "error" in d or not d.get("timestamp"):
        return None
    df = pd.DataFrame({
        "date": pd.to_datetime(d["timestamp"], unit="s", utc=True).tz_convert("Asia/Kolkata").tz_localize(None),
        "open": d.get("open"), "high": d.get("high"), "low": d.get("low"),
        "close": d.get("close"), "volume": d.get("volume"),
        "adjclose": d.get("adjclose") if d.get("adjclose") else d.get("close"),
    })
    df = df.dropna(subset=["adjclose"]).reset_index(drop=True)
    if df.empty:
        return None
    # Scale OHLC by the adjustment factor so highs/lows stay comparable to adjclose.
    factor = df["adjclose"] / df["close"].replace(0, np.nan)
